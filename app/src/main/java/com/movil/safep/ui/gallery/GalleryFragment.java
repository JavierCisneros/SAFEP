...
public class GalleryFragment extends Fragment {
       ...
       act.setOnClickListener(new View.OnClickListener() {
           ...
           public void onClick(View view) {
               ...
               if(cat.equals("Gastos")) {
                   if(valores.size()==0){
                       Toast.makeText(getContext(), "No hay gastos registrados este dia", Toast.LENGTH_SHORT).show();
                   }
               ...
               {
                   if(valores2.size()==0){
                       Toast.makeText(getContext(), "No hay ingresos registrados este dia", Toast.LENGTH_SHORT).show();
                   }
                   for (int i = 0; i < valores2.size(); i++) {
                       Entries.add(new PieEntry(0 + valores2.get(i), labels2.get(i)));
                       System.out.println("Ingreso:" + valores2);
    ...
    public void descarga(){
        ...
        for(int j = 1; j < res.getStringArray(R.array.Categorias).length; j++) {
            Query lista = mDatabase.child("Users").child("ID:" + id).child("Gastos").child("Year:" + yearG).child("Month:" + monthG).child("Day:" + dayG).child("Categoria:" + LC[j]);
            final int finalJ = j;
        ...
        for(int j = 1; j < res.getStringArray(R.array.Ingresos).length; j++) {
            Query lista2 = mDatabase.child("Users").child("ID:" + id).child("Ingresos").child("Year:" + yearG).child("Month:" + monthG).child("Day:" + dayG).child("Categoria:" + LC2[j]);
            final int finalJ = j;
